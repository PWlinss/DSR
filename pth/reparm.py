...
dict = torch.load('models/DRP-LKDN_del_x4.pth')
...
for i in range(len(blk_name)):
    if blk_name[i].endswith('r.pw.weight'):
        ...
        weight_idt = torch.zeros(56, 56, 1, 1)
        for j in range(56):
        ...
        weight_idt = torch.zeros(56, 1, 3, 3)
        for j in range(56):
...
torch.save(dict, 'models/Rep-DRP-LKDN_x4.pth')

dict = torch.load('models/Rep-DRP-LKDN_x4.pth')
